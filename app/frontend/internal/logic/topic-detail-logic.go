...
func (l *TopicDetailLogic) TopicDetail(req types.TopicItemReq) (resp *types.TopicDetailResply, err error) {
	...
	result := l.svcCtx.Engine.Table(SnsTopicM.TableName()).Select(SnsTopicM.TableName()+".*,"+SnsTopicUserM.TableName()+".cate_id").Joins("left join "+SnsTopicUserM.TableName()+" on "+SnsTopicM.TableName()+".id = "+SnsTopicUserM.TableName()+".topics_id").Where(SnsTopicM.TableName()+".id = ?", req.TopicId).Find(&SnsTopicData)
